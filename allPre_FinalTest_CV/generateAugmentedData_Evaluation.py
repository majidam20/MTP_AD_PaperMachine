...
def generateAugDTW(yX, addAug=True , numberOfAug=14):
    ...
    print(f"\n Start running time Data Augmentation With DTW Approach: {datestr} ,++++++++++++++++++++++++++++++:\n")


    yXtrain = yX

    MSEAllAuged = np.empty([0, 3])
    ...
    for i in range(1, numberOfAug):
        ...
        MSEAllAuged = np.append(MSEAllAuged, [[mse.mean(), mse.min(), mse.max()]], axis=0)
    ...
    print(f"\n \n Generated this amount: {yX_TimeWarping.shape[0]} ,Synthetic data With DTW Approach\n")




    print(f"\nMSE All generated Augmented data:\n"
          f"mean: {np.round(np.float(MSEAllAuged[:, 0].mean(axis=0)), 6)} \n"
          f"min:  {np.round(np.float(MSEAllAuged[:, 1].mean(axis=0)), 6)} \n"
          f"max:  {np.round(np.float(MSEAllAuged[:, 2].mean(axis=0)), 6)} \n"
          )
    ...
    datestr = time.strftime("%y%m%d_%H%M%S")
    print(f"\n End running time Data Augmentation With DTW Approach: {datestr} ,++++++++++++++++++++++++++++++. \n")
...
def generateAugAE(yX,makeNoiseByJitter,pathSavingModelAE,addAug=True,numberOfAug=14,saveModelAug=False):
        ...
        for i in range(1,numberOfAug):
            ...
            if i==1:
                ...
                modelAE.compile(optimizer=adam, loss='mse' , metrics = ['mse'])
                ...
                if saveModelAug == True:
                    chkp = ModelCheckpoint(pathSavingModelAE, save_best_only=True, monitor='mse', verbose=2)
                    modelAE.fit(input_X, input_X,
                                epochs=epochs,
                                batch_size=batch,
                                verbose=0, use_multiprocessing=True,
                                shuffle=FitShuffle,callbacks=[chkp])




                ### Fit modelAE
                if saveModelAug == False:
                    modelAE.fit(input_X, input_X,
                                epochs=epochs,
                                batch_size=batch,
                                verbose=0, use_multiprocessing=True,
            ...
            if saveModelAug == True:
                savedModelAE = tf.keras.models.load_model(pathSavingModelAE)
                forAug = savedModelAE.predict(input_X, verbose=1)
        ...
        ###Delete model after finishing all generated synthetic data because in TensorFlow, when we are using several models in a loop, some graphs and layers' name will still be kept, but in above already modelAE was saved
        del modelAE
        gc.collect()
