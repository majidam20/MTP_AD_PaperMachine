...
class mainClass_Evaluation:
    ...
    def selectModelTypeFunc(self, xtrain ,ModelFinalTestEvaluation, flagR1,r1,r2, selectModelType="DNN"):

        if selectModelType == "DNN":

            ModelFinalTestEvaluation.add(Dense(xtrain.shape[1] * 2, activation='tanh', input_dim=xtrain.shape[1]))  # relu, tanh
            ModelFinalTestEvaluation.add(Dense(xtrain.shape[1] * 1.6, activation='tanh',
                                       kernel_regularizer=l1(r1) if flagR1 else l2(r2)
                                       ))
            ModelFinalTestEvaluation.add(Dense(xtrain.shape[1] * .31, activation='tanh',
                                       kernel_regularizer=l1(r1) if flagR1 else l2(r2)
                                       ))
            ModelFinalTestEvaluation.add(Dense(1, activation='sigmoid'))
        ...
        if selectModelType=="LSTM":
            ...
            ###LSTM Model
            ModelFinalTestEvaluation.add(LSTM(xtrain.shape[2]*2, activation='tanh', input_shape=(timesteps, n_features)))### tanh, relu

            ModelFinalTestEvaluation.add(Dense(int(xtrain.shape[2]*1.6)
                                     , activation='tanh'
                                     , kernel_regularizer=l1(r1) if flagR1 else l2(r2)
                                     ))
            ModelFinalTestEvaluation.add(Dense(int(xtrain.shape[2] * .31), activation='tanh',
                                       kernel_regularizer=l1(r1) if flagR1 else l2(r2)
                                       ))
        ...
        return ModelFinalTestEvaluation
   ...
   ##############################****** Ploting Part ******##############################
    ...
    def pltLossVal(self,historyLoss, shiftNumber,winLength, epochs, pathSavingPlotsPerRunning,baseFileName,batch,flagSeed=True,ylim=.5,r2="",addAug=True, selectModelType= "DNN"):
        ...
        plt.title(f'Loss-Epoch Model FinalTestEvaluation',pad=20,fontdict= {'weight' : 'bold', 'size': 8})
        plt.ylabel('Loss',labelpad=10,fontdict= {'weight' : 'bold'})
        plt.ylim(0, ylim)  # .2
        plt.xlabel('Epoch',labelpad=10,fontdict= {'weight' : 'bold'})

        plt.savefig(pathSavingPlotsPerRunning + "/" + f"loss_FinalTest_shiftNumber{shiftNumber}_winLength{winLength}_Model{selectModelType}_Epochs{epochs}_Batch{batch}_Ridge{r2}_flagSeed{flagSeed}_addAug{addAug}_{baseFileName}.png",
                    dpi=300, format='png')
        #plt.show()
    ...
    def pltPrRe(self,yfinalTest, yPred, shiftNumber, winLength,epochs,pathSavingPlotsPerRunning,baseFileName,batch,flagSeed=True,r2="",addAug=True, selectModelType= "DNN"):

        precision_rt, recall_rt, threshold_rt = precision_recall_curve(yfinalTest, yPred)
        plt.figure()
        plt.plot(precision_rt[1:], recall_rt[1:], label="Precision-Recall (Anomaly 1)", linewidth=2, color="green")
        plt.title(f'Precision-Recall FinalTestEvaluation',pad=20,fontdict= {'weight' : 'bold','size': 8})
        plt.xlabel('Recall',labelpad=10,fontdict= {'weight' : 'bold'})
        plt.ylabel('Precision',labelpad=10,fontdict= {'weight' : 'bold'})
        plt.legend(loc="best")

        plt.savefig(pathSavingPlotsPerRunning + "/" + f"Precision&Recall_FinalTest_shiftNumber{shiftNumber}_winLength{winLength}_Model{selectModelType}_Epochs{epochs}_Batch{batch}_Ridge{r2}_flagSeed{flagSeed}_addAug{addAug}_{baseFileName}.png",
                    dpi=300, format='png')
        #plt.show()
    ...
    ### To print Average rates such as threshold, precision, recall  at the end of running
    def printPrReThreshold(self,yfinalTest, yPred, thresholdDistance = 1):

        precision_rt, recall_rt, threshold_rt = precision_recall_curve(yfinalTest, np.round(yPred, thresholdDistance))

        threshold_rt2, precision_rt2, recall_rt2 = threshold_rt.reshape(-1,1).mean(), precision_rt[1:].reshape(-1,1).mean(), recall_rt[1:].reshape(-1,1).mean()

        print("Average threshold_rt Final Test: ", np.round(threshold_rt2,2))
        print("Average precision_rt Final Test: ", np.round(precision_rt2,2))
        print("Average recall_rt    Final Test: ", np.round(recall_rt2,2), "\n\n")
    ...
    def printConfMatrix(self,yfinalTest,yPredCategorical, labelsValues=[0, 1]):

        print(f"confusion_matrix FinalTest: ")

        tn, fp, fn, tp = confusion_matrix(yfinalTest, yPredCategorical, labels=labelsValues).ravel()
    ...
    def pltConfMatrix(self,yfinalTest, yPredCategorical,shiftNumber,winLength,LABELS,epochs,pathSavingPlotsPerRunning,baseFileName,batch,flagSeed=True,figsizeValues=(6, 6), labelsValues=[0, 1],r2="",addAug=True, selectModelType= "DNN"):

        conf_matrix = confusion_matrix(yfinalTest, yPredCategorical, labels=labelsValues)
        ...
        plt.title(f"Confusion matrix FinalTest",pad=20,fontdict= {'weight' : 'bold'})
        plt.ylabel('True class',labelpad=20,fontdict= {'weight' : 'bold'})
        plt.xlabel('Predicted class',labelpad=20,fontdict= {'weight' : 'bold'})

        plt.savefig(pathSavingPlotsPerRunning + "/" + f"ConfusionMatrix_FinalTest_shiftNumber{shiftNumber}_winLength{winLength}_Model{selectModelType}_Epochs{epochs}_Batch{batch}_Ridge{r2}_flagSeed{flagSeed}_addAug{addAug}_{baseFileName}.png",dpi=300, format='png')
        #plt.show()
    ...
    def pltDfCompSnsDist(self,Xtrain, Xauged,shiftNumber,winLen, pathSavingPlotsPerRunning, baseFileName, flagSeed, w=20, h=200, bins=20, epochs=1000, selectModelType= "DNN"):
        ...
        for (norm, abnorm,
            ...
            axes[row, col].set_ylabel(f"Col_{norm+1}", labelpad=5, fontweight="bold", fontsize=7)
        ...
        plt.savefig(pathSavingPlotsPerRunning + "/" + f"CompSnsDist_FinalTest_shiftNumber{shiftNumber}_winLength{winLen}_Model{selectModelType}_Epochs{epochs}_flagSeed{flagSeed}_{baseFileName}.png",dpi=300, format='png')
        #plt.show()
