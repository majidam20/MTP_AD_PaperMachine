...
train_data_file = os.path.join("data", "forAugDS", "dfpShifted5_ForAug_AllTrainTest_201204_161806.csv")
test_data_file = os.path.join("data","forAugDS", "dfpShifted5_ForAug_AllTrainTest_201204_161806.csv")
x_train, y_train, x_test, y_test = read_data_sets(train_data_file, "", test_data_file, "", delimiter=",")
...
df_jitter2     .to_csv(f"data/augedPaperMachine/df_jitter2_{datestr}.csv",index=False,header=None)
...
# df_random_guided_warp     =pd.DataFrame(np.array(np.squeeze(aug.random_guided_warp(x_train,y_train),axis=2)))
# df_random_guided_warp     .to_csv(f"data/augedPaperMachine/df_random_guided_warp_{datestr}.csv",index=False,header=None)
