...
def GenerateAug_NN_Rolling(yX,foldNum,AugedWithJittering,flagLbl2=True,model=0,jitterNum4Lbl1=10,jitterNum4Lbl2=20):
    ...
    for Label in range(2,3):### Iterate over label types that are 1,2, If End range is 2 then just Generate aug labeled 1, if End range is 3 then Generate aug labeled 1 and 2
        ...
        for iterLabel in range(1,3):### Iterate over Number of desired aug for each label types that are 1,2
            if iterLabel==1 and Label==1:# First Iteration For Label1
                ...
                for jitterIter4Lbl1 in range(1, jitterNum4Lbl1):
                    jitter = np.concatenate((X1[:,0].reshape([X1.shape[0],1]), jitterFun(X1[:,1:], sigma=0.000001 + jitterIter4Lbl1 / 100000)),  axis=1)
            ...
            if iterLabel == 1 and Label==2:# First Iteration For Label2
                ...
                for jitterIter4Lbl2 in range(1, jitterNum4Lbl2):
                    jitter = np.concatenate(
                        (X2[:, 0].reshape([X2.shape[0], 1]), jitterFun(X2[:, 1:], sigma=0.000001 + jitterIter4Lbl2 / 100000)), axis=1)
                    jitters = np.append(jitters, jitter, axis=0)
                forAug = jitters
                jitter = 0
                jitters=np.empty([0,yX.shape[1]])
            ...
            ###Generate synthetic data just with Jittering
            if AugedWithJittering==True:
                if Label==1:
                    y1 = np.repeat(np.array([[1]]), input_X.shape[0], axis=0)
                    yX1 = np.concatenate((y1, input_X), axis=1)
                    yXPred1 = np.append(yXPred1, yX1, axis=0)
                    continue
                    #yPred = np.empty([0, yX.shape[1] - 1])
                    # pd.DataFrame(yXPred1).to_csv(pathDataRolLbl2+f'dfp5_RolLabel2_AugedNN_Lbl{Label}_Iter{iterLabel}_{datestr}.csv',index=None,header=None)

                if Label==2:
                    y2 = np.repeat(np.array([[2]]), input_X.shape[0], axis=0)
                    yX2 = np.concatenate((y2, input_X), axis=1)
            ...
            epochs = 1000#00#00#2000#2#000#3#50#100  # 3#10#50#100#3
            batch = 16
            lr = 0.0001
            ...
            if iterLabel==1:
                model = Sequential()
                model.add(Dense(neurons1, activation='tanh',input_dim=input_X.shape[1])) #input_X.shape[1] # ,input_shape=(input_XActual.shape[1],),
                model.add(Dense(neurons2, activation='tanh'))  # ,kernel_initializer=initializer,
                #model.add(Dense(30, activation='tanh'))#Latent AE
                model.add(Dense(295, activation='tanh'))
                #model.add(Dense(neurons2, activation='tanh'))#AE
                #model.add(Dense(neurons1, activation='tanh'))#AE, input_X.shape[1]
            ...
            #mse = np.mean(np.power(input_X - model.predict(input_X), 2), axis=0)
            print(f"\nData Augmentation MSE Label{Label}, iter{iterLabel}==> mean: {mse.mean()}, min: {mse.min()}, max: {mse.max()} \n")

            # break
            # pd.DataFrame(mse.reshape(mse.shape[0],1)).to_csv(
            #     pathDataRolLbl2 + f'MSE_RolLabel2_AugedNN_Lbl{Label}_Iter{iterLabel}_{datestr}.csv', index=None,
            #     header=None)
            #break
            if Label==1:
            ...
            if Label==2:
                ...
                yXPred2 = np.append(yXPred2, yX2, axis=0)
                yPred = np.empty([0, yX.shape[1] - 1])
                # pd.DataFrame(yXPred2).to_csv(
                #     pathDataRolLbl2 + f'dfp5_RolLabel2_AugedNN_Lbl{Label}_Iter{iterLabel}_{datestr}.csv', index=None,
                #     header=None)
