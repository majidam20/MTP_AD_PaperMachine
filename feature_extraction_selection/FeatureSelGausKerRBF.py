...
dfpRaw2 = np.asarray(dfpRaw2).astype('float16')
...
permGausRBF = permutation_importance(gpc, df[:,1:], df[:,0])

sorted_idx = permGausRBF.importances_mean.argsort()


print(f"sortedFeatSel:\n {sorted_idx}\n")

print(f"importances:\n {pd.DataFrame(np.round(permGausRBF.importances,4))}\n")

print(f"permGausRBF.importances_mean:\n {pd.DataFrame(np.round(permGausRBF.importances_mean,4))}\n")
